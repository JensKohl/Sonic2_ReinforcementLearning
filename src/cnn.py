"""
CNN Agent Architecture for processing visual environment states.
"""
import torch
...
class NatureCNN(nn.Module):
    """
    CNN architecture described in 'Human-level control through deep
    ...
    def __init__(self, input_shape, features_dim=512):
        ...
        # We calculate the number of features after flattening so the next
        # layer knows its input size.
        with torch.no_grad():
            dummy_input = torch.as_tensor(np.zeros((1, *input_shape))).float()
            n_flatten = self.cnn(dummy_input).shape[1]

        # --- THE FEATURE BED ---
        # A fully connected layer that condenses the 3136 raw features into
        # 512 high-level "concepts."
