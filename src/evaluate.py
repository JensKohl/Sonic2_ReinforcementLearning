"""
Standard Evaluation Script for the Sonic 2 RL Agent.
"""
    ...
    try:
        ...
        with torch.inference_mode():
            while steps < 8000:  # Limit length
                ...
                obs_tensor = (
                    torch.as_tensor(next_obs, device=device).unsqueeze(0).float()
                )

                if steps % 50 == 0:
                    vx = info.get("x", 0) - info.get("prev_x", info.get("x", 0))
                    print(
                        f"Step {steps} | Pos: ({info.get('x', 0)}, {info.get('y', 0)}) | "
                        f"VX: {vx} | Act: {cpu_action} | Rings: {info.get('rings', 0)}"
                    )
                ...
                if info.get("level_end_bonus", 0) > 0:
                    ...
                    obs, _ = env.reset()
                    obs_tensor = (
                        torch.as_tensor(obs, device=device).unsqueeze(0).float()
                    )
                    total_reward = 0
                    steps = 0
                    continue
                ...
                if terminated or truncated:
                    ...
                    print(
                        f"Episode Ended | Reason: {reason} | "
                        f"Final Pos: ({info.get('x', 0)}, {info.get('y', 0)}) | "
