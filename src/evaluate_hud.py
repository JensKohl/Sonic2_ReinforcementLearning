"""
HUD-Enabled Evaluation Script for visual playback.
"""
...
def make_env_eval(
    game, state, stack_frames=4, render=False, max_steps=None, max_stagnant_steps=None
):
    def _init():
...
def draw_info_panel(frame, action_index, reward_sum, value_est=None, episode_num=1):
    ...
    canvas = cv2.copyMakeBorder(
        frame, 0, panel_height, 0, 0, cv2.BORDER_CONSTANT, value=panel_color
    )
    ...
    for i in range(5, 10):
        ...
        cv2.putText(
            canvas,
            label,
            (start_x + (i - 5) * col_spacing, start_y + row_spacing),
            font,
            font_scale,
            color,
            thick,
...
def evaluate_with_hud(args):
    ...
    env = make_env_eval(
        ...
        render=False,
        max_steps=max_steps,
        max_stagnant_steps=max_stagnant,
    )()
        ...
        with torch.no_grad():
            while True:
                ...
                if step_counter % 60 == 0:
                    ...
                    print(
                        f"Step {step_counter} | X: {curr_x} | Y: {curr_y} | "
                        f"Rings: {info.get('rings', -1)} | Bonus: {bonus}"
                    )
                ...
                # Draw HUD
                hud_frame = draw_info_panel(
                    frame_bgr, action_scalar, total_reward, value_scalar, episode_count
                )

                # Scale up for easier viewing (2x)
                display_frame = cv2.resize(
                    hud_frame, (0, 0), fx=2.0, fy=2.0, interpolation=cv2.INTER_NEAREST
                )
                ...
                if info.get("level_end_bonus", 0) > 0:
                    ...
                    print(
                        f"Episode {episode_count} Victory! Reward: {total_reward:.2f}"
                    )
                ...
                if terminated or truncated:
                    print(
                        f"Episode {episode_count} End (Terminated/Truncated). Reward: {total_reward:.2f}"
                    )
                    obs, _ = env.reset()
                    obs_tensor = torch.tensor(obs, device=device).unsqueeze(0).float()
                    total_reward = 0.0
                    ...
                    obs_tensor = (
                        torch.tensor(next_obs, device=device).unsqueeze(0).float()
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", type=str, required=True, help="Path to the .pth model file"
    )
    parser.add_argument(
        "--stochastic",
        action="store_true",
        help="Use stochastic sampling (randomness) instead of deterministic argmax",
    )
    parser.add_argument(
        "--infinite",
        action="store_true",
        help="Disable time limits and stagnation checks",
    )
