"""
Proximal Policy Optimization (PPO) logic and Rollout Buffer structure.
"""
...
class RolloutBuffer:
    ...
    def __init__(self, buffer_size, num_envs, obs_shape, action_shape, device):
        ...
        self.actions = torch.zeros(
            (buffer_size, num_envs) + action_shape, dtype=torch.long
        ).to(
            device
        )  # Actions must be Long (integers)
    ...
    def add(self, obs, action, logprob, reward, done, value):
        ...
        self.values[self.pos] = value.reshape(
            -1
        )  # Flatten value array to match batch dimension
    ...
    def update(self, buffer, minibatch_size=256, epochs=4):
        ...
        for _ in range(epochs):
            ...
            for start in range(0, len(inds), minibatch_size):
                ...
                pg_loss = torch.max(
                    pg_loss1, pg_loss2
                ).mean()  # Take the conservative (min benefit) estimate
                ...
                )  # Safety: don't let updates be too big
