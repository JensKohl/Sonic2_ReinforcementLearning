"""
Utility script to autonomously record a completed game level to MP4.
"""
...
def record_victory(model_path, output_path, threshold=10000, playback_fps=60.0):
    ...
    print(
        f"Loading {model_path} to record victory (Threshold={threshold}, FPS={playback_fps})..."
    )
    ...
    for ep in range(100):  # 100 attempts to get a win
        ...
        if is_victory:
            ...
            video_writer = cv2.VideoWriter(
                output_path, fourcc, playback_fps, (width, height)
            )
            for f in frames:
