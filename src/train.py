"""
Training entry point using Proximal Policy Optimization (PPO).
            ...
            try:
                ...
                if temp >= self.threshold:
                    ...
                    raise KeyboardInterrupt(
                        "GPU too hot"
                    )  # Using KeyboardInterrupt to stop clean-ish
...
def train():
    ...
    if args.checkpoint:
        ...
        if os.path.exists(latest_cp):
            ...
            print(
                "Warning: --resume flag set but no latest_checkpoint.pth found. Starting from scratch."
            )
    ...
    if checkpoint_path:
        ...
        if isinstance(loaded_data, dict) and "global_step" in loaded_data:
            ...
            print(
                f"Resuming run '{run_name}' at update {start_update}, global step {global_step}"
            )
        else:
    ...
    writer = SummaryWriter(
        ...
        if (checkpoint_data and "global_step" in checkpoint_data)
        else None,
    )
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=50000, save_path="models/checkpoints", run_name=run_name
    )
    ...
    try:
        ...
        print(
            f"Starting training for {num_updates} updates (Current: {start_update}/{num_updates})..."
        )
        ...
        for update in tqdm(range(start_update, num_updates + 1), desc="Training"):
            ...
            for _ in range(num_steps):
                global_step += (
                    1 * num_envs
                )  # Track total frames processed across all environments
                ...
                )  # Combine termination (death/win) and truncation (time)
                ...
                if "final_info" in infos:
                    for info in infos["final_info"]:
                        if info and "episode" in info:
                            ...
                            writer.add_scalar(
                                "charts/episodic_return", ep_reward, global_step
                            )
                            writer.add_scalar(
                            ...
                            is_victory = (
                                1.0 if info.get("level_end_bonus", 0) > 0 else 0.0
                            )
                            ...
                            if best_model_callback.update_best(agent, ep_reward):
                                tqdm.write(
                                    f"--> [NEW BEST] Step: {global_step} | Reward: {ep_reward:.2f}"
                                )
            ...
            train_stats[
                "agent_state"
            ] = agent.state_dict()  # Capture state for any potential checkpoint
            ...
            # Periodic Checkpoint
            checkpoint_callback.on_update_with_data(
                update, global_step, agent, optimizer
            )
